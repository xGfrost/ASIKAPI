...
export async function saveAiConsultationNotes(req: Request, res: Response) {
  ...
  const { notes_text, diarization_json, risk_analysis_json, mitigation_recommendations } = req.body ?? {};

  const notes = await prisma.ai_consultation_notes.upsert({
    where: { consultation_id: cid },
    update: {
      notes_text, diarization_json, risk_analysis_json,
      mitigation_recommendations, updated_at: new Date()
    },
    create: {
      consultation_id: cid,
      psychologist_id: c.psychologist_id,
      patient_id: c.patient_id,
      notes_text, diarization_json, risk_analysis_json, mitigation_recommendations
    }
  });
...
export async function getAiConsultationNotes(req: Request, res: Response) {
  ...
  const notes = await prisma.ai_consultation_notes.findUnique({ where: { consultation_id: cid } });
  if (!notes) return res.status(404).json({ error: { message: "AI notes not found" } });
...
export async function updateAiConsultationNotes(req: Request, res: Response) {
  ...
  const updated = await prisma.ai_consultation_notes.update({
    where: { consultation_id: cid },
    data: { notes_text, diarization_json, risk_analysis_json, mitigation_recommendations, updated_at: new Date() }
  });
