...
function toBigInt(x: number | string | bigint) {
...
export async function assignPsychologistSpecialties(req: Request, res: Response) {
  ...
  if (!idParam) return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  const psychologistId = BigInt(idParam);

  // auth: admin atau owner
  const actor = (req as any).user;
  if (!actor) return res.status(401).json({ error: { message: "Unauthorized" } });
  if (actor.role !== "admin" && !(actor.role === "psychologist" && actor.id === psychologistId)) {
  ...
  const found = await prisma.specialties.findMany({ where: { id: { in: ids } }, select: { id: true } });
  const foundIds = new Set(found.map(s => s.id.toString()));
  const missing = ids.filter(i => !foundIds.has(i.toString()));
  if (missing.length > 0) {
    return res.status(400).json({ error: { message: `Unknown specialty id(s): ${missing.join(", ")}` } });
  }

  // replace all
  await prisma.$transaction(async (tx) => {
    await tx.psychologist_specialties.deleteMany({ where: { psychologist_id: psychologistId } });
    if (ids.length > 0) {
      await tx.psychologist_specialties.createMany({
        data: ids.map((sid) => ({ psychologist_id: psychologistId, specialty_id: sid })),
        skipDuplicates: true,
      });
    }
  });
...
export async function removePsychologistSpecialty(req: Request, res: Response) {
  ...
  if (!idParam || !sidParam) {
    return res.status(400).json({ error: { message: "Psychologist ID and Specialty ID are required" } });
  }
  const psychologistId = BigInt(idParam);
  const specialtyId = BigInt(sidParam);
