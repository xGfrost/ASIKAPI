...
function forbidIfNotOwnerOrAdmin(actor: any, psyId: bigint) {
  return !(actor.role === "admin" || (actor.role === "psychologist" && actor.id === psyId));
}
...
export async function getPsychologistDashboard(req: Request, res: Response) {
  ...
  if (forbidIfNotOwnerOrAdmin(actor, psyId)) return res.status(403).json({ error: { message: "Forbidden" } });
...
export async function listPsychologistConsultationsByStatus(req: Request, res: Response) {
  if (!req.params.id) return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  const psyId = BigInt(req.params.id);
  const actor = (req as any).user;
  if (forbidIfNotOwnerOrAdmin(actor, psyId)) return res.status(403).json({ error: { message: "Forbidden" } });
...
export async function getPsychologistPaymentsSummary(req: Request, res: Response) {
  if (!req.params.id) return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  const psyId = BigInt(req.params.id);
  const actor = (req as any).user;
  if (forbidIfNotOwnerOrAdmin(actor, psyId)) return res.status(403).json({ error: { message: "Forbidden" } });
...
export async function listPsychologistIntakeFormsDash(req: Request, res: Response) {
  if (!req.params.id) return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  const psyId = BigInt(req.params.id);
  const actor = (req as any).user;
  if (forbidIfNotOwnerOrAdmin(actor, psyId)) return res.status(403).json({ error: { message: "Forbidden" } });
