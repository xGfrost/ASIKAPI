...
type ConsultationItemDTO = Prisma.consultationsGetPayload<{
  include: { patient: true };
}>;
type PsychologistConsultationsResponse = { items: ConsultationItemDTO[] };
...
type IntakeFormListItemDTO = Prisma.intake_formsGetPayload<{
  select: {
    id: true;
...
type IntakeFormsListResponse = { items: IntakeFormListItemDTO[] };

// GET /psychologists/:id/ai-reports
type LatestNotesDTO = Prisma.ai_consultation_notesGetPayload<{
  select: {
    consultation_id: true;
    notes_text: true;
...
export async function getPsychologistDashboard(
  req: Request,
  res: Response<PsychologistDashboardResponse | ErrorResponse>
) {
  if (!req.params.id) {
...
export async function listPsychologistConsultationsByStatus(
  req: Request,
  res: Response<PsychologistConsultationsResponse | ErrorResponse>
) {
  if (!req.params.id) {
    return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  }
  ...
  const where: Prisma.consultationsWhereInput = { psychologist_id: psyId };
...
export async function getPsychologistPaymentsSummary(
  req: Request,
  res: Response<PaymentsSummaryResponse | ErrorResponse>
) {
  if (!req.params.id) {
    return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  }
...
export async function listPsychologistIntakeFormsDash(
  req: Request,
  res: Response<IntakeFormsListResponse | ErrorResponse>
) {
  if (!req.params.id) {
    return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  }
...
// GET /psychologists/:id/ai-reports
export async function getPsychologistAiReports(
  req: Request,
  res: Response<AiReportsResponse | ErrorResponse>
) {
  if (!req.params.id) {
    return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  }
