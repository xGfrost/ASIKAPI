...
function scoreFromText(t: string | null | undefined) {
  if (!t) return 0;
  const s = t.toLowerCase();
  let score = 0;
  const high = ["bunuh diri", "suicide", "self-harm", "membunuh", "takut mati", "halusinasi"];
  const med  = ["depresi", "panic", "panic attack", "cemas", "anxiety", "insomnia"];
  high.forEach(k => { if (s.includes(k)) score += 3; });
  med.forEach(k => { if (s.includes(k)) score += 2; });
  if (s.includes("sering") || s.includes("parah")) score += 1;
...
export async function runAiIntakeAnalysis(req: Request, res: Response) {
  ...
  const s = scoreFromText(
    [form.symptoms_text, form.duration_text, form.triggers_text, form.additional_info].join(" ")
  );
  const level = riskLevel(s);
  const flags = {
    keywords: { score: s, level },
    sources: ["symptoms_text","duration_text","triggers_text","additional_info"]
  };
...
export async function getAiIntakeAnalysis(req: Request, res: Response) {
    if (!req.params.id) return res.status(400).json({ error: { message: "Psychologist ID is required" } });
  const cid = BigInt(req.params.id);
  const actor = (req as any).user;
  ...
  const ai = await prisma.ai_intake_analysis.findUnique({ where: { intake_form_id: form.id } });
  if (!ai) return res.status(404).json({ error: { message: "AI analysis not found" } });
